...
public class SettingsPage extends BasePage {
  ...
  @FindBy(
      css =
          "button.v-btn.v-app-bar-nav-icon .mdi-menu.mdi.v-icon.notranslate.v-theme--default-upc.v-icon--size-default")
    ...
    try {
      ...
      if (rows.size() > index) {
        ...
        WebElement deleteButton =
            row.findElement(
                By.xpath(
                    ".//i[@class='mdi-delete mdi v-icon notranslate v-theme--myCustomLightTheme v-icon--size-default text-error v-icon--clickable']"));
        deleteButton.click();
        // Підтвердження видалення
        WebDriverWait wait =
            new WebDriverWait(
                getDriver(), Duration.ofSeconds(10)); // Використання старого конструктора
