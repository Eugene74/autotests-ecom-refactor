...
public class InstalmentPaymentCardPage extends BasePage {
  ...
  private String setCardData(String[] card) {
    ...
    getDriver()
        .findElement(
            By.xpath("/html/body/div/div/main/section/div/form/div/div[1]/div[2]/div/div/input"));
    String orderID = order.getText();
    System.out.println(
        "Order ID captured: " + orderID); // Додаткове логування для перевірки значення orderID
    Waiters.sleep(1500);
    ...
    String expiryDate =
        card[2]
            + "/"
            + card[1].substring(2); // card[2] - місяць, card[1] - рік, беремо останні 2 цифри року
