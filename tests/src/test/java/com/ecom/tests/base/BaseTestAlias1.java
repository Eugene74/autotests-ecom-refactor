...
public class BaseTestAlias1
    extends BaseApiTest { // todo потом переделать(или убрать) это промежуточный BaseTestAlias1?
  ...
  protected String verifyXmlResponse(String xml, SoftAssert softAssertion, String tagName)
      throws Exception {
    ...
    boolean isGuidNotEmpty =
        guidNodes.getLength() > 0 && !guidNodes.item(0).getTextContent().trim().isEmpty();
