...
import static com.ecom.core.config.EnvData.URL_ALIAS_2_RESOLVE;
import static com.ecom.core.config.XMLAliasResource.XML_RESOLVE_ALIAS2_RESOURCE;
...
public class ResolveAlias2 extends BaseTestAlias2 {
  ...
  public void testResolveAlias2() throws Exception {
    ...
    HttpResponse response = sendPostRequest(URL_ALIAS_2_RESOLVE, xmlContent);
  ...
  private void verifyResolveAliasResponse(String xml, SoftAssert softAssertion) throws Exception {
    ...
    boolean isResolveAliasResultNotEmpty =
        resolveAliasResultNodes.getLength() > 0
            && resolveAliasResultNodes.item(0).getTextContent().trim().length() > 0;
    ...
    if (isCodePresent) {
      ...
      if (profileNodes.getLength() > 0) {
        ...
        if (firstNameLocalNodes.getLength() > 0) {
          System.out.println(
              "firstNameLocal: " + firstNameLocalNodes.item(0).getTextContent().trim());
        }
        ...
        if (lastNameLocalNodes.getLength() > 0) {
          System.out.println(
              "lastNameLocal: " + lastNameLocalNodes.item(0).getTextContent().trim());
        }
        ...
        if (middleNameLocalNodes.getLength() > 0) {
          System.out.println(
              "middleNameLocal: " + middleNameLocalNodes.item(0).getTextContent().trim());
        }
        if (preferredNameNodes.getLength() > 0) {
          System.out.println(
              "preferredName: " + preferredNameNodes.item(0).getTextContent().trim());
        }
      }
      ...
      if (identificationNodes.getLength() > 0) {
        ...
        if (verificationDetailsNodes.getLength() > 0) {
          ...
          if (authDateTimeNodes.getLength() > 0) {
            System.out.println(
                "authDateTime: " + authDateTimeNodes.item(0).getTextContent().trim());
          }
          if (authMethodReferenceNodes.getLength() > 0) {
            System.out.println(
                "authMethodReference: " + authMethodReferenceNodes.item(0).getTextContent().trim());
          }
          if (creationDateTimeNodes.getLength() > 0) {
            System.out.println(
                "creationDateTime: " + creationDateTimeNodes.item(0).getTextContent().trim());
          }
          if (verifiedEmailNodes.getLength() > 0) {
            System.out.println(
                "verifiedEmail: " + verifiedEmailNodes.item(0).getTextContent().trim());
          }
          if (verifiedPhoneNodes.getLength() > 0) {
            System.out.println(
                "verifiedPhone: " + verifiedPhoneNodes.item(0).getTextContent().trim());
          }
        }
      }
    }
