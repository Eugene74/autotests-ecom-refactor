...
import static com.ecom.core.config.EnvData.URL_ALIAS_2_GET_ALL_PAYMENT_CRED;
import static com.ecom.core.config.XMLAliasResource.XML_GET_ALL_PAYMENT_CREDENTIALS_ALIAS2_RESOURCE;
...
public class GetAllPaymentCredentialsAlias2 extends BaseTestAlias2 {
  ...
  public void testGetAllPaymentCredentialsAlias2() throws Exception {
    ...
    HttpResponse response = sendPostRequest(URL_ALIAS_2_GET_ALL_PAYMENT_CRED, xmlContent);
  ...
  private void verifyGetAllPaymentCredentialsResponse(String xml, SoftAssert softAssertion)
      throws Exception {
    ...
    NodeList getAllPaymentCredentialsResultNodes =
        doc.getElementsByTagName("GetAllPaymentCredentialsResult");
    boolean isGetAllPaymentCredentialsResultNotEmpty =
        getAllPaymentCredentialsResultNodes.getLength() > 0
            && getAllPaymentCredentialsResultNodes.item(0).getTextContent().trim().length() > 0;
    System.out.println(
        "GetAllPaymentCredentialsResult is not empty: " + isGetAllPaymentCredentialsResultNotEmpty);
    softAssertion.assertTrue(
        isGetAllPaymentCredentialsResultNotEmpty, "<GetAllPaymentCredentialsResult> is empty");
    ...
    if (isCodePresent) {
      ...
      if (expirationDateNodes.getLength() > 0) {
        System.out.println(
            "ExpirationDate: " + expirationDateNodes.item(0).getTextContent().trim());
      }
      ...
      if (lastFourDigitsNodes.getLength() > 0) {
        System.out.println(
            "LastFourDigits: " + lastFourDigitsNodes.item(0).getTextContent().trim());
      }
