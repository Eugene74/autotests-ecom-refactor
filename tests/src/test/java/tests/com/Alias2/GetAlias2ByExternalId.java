...
import static com.ecom.core.config.EnvData.URL_ALIAS_2_GET_BY_EXTERNAL_ID;
...
public class GetAlias2ByExternalId extends BaseTestAlias2 {
  ...
  public void testGetAlias2ByExternalId() throws Exception {
    ...
    String externalId =
        getAliasIdFromDatabase(SharedDataStore.trackingId, QUERY, "ALIAS_EXTERNAL_ID");
    if (externalId == null) {
    ...
    HttpResponse response = sendPostRequest(URL_ALIAS_2_GET_BY_EXTERNAL_ID, xmlContent);
  ...
  private void verifyGetAliasByExternalIdResponse(String xml, SoftAssert softAssertion)
      throws Exception {
    ...
    boolean isGetByExternalIdResultNotEmpty =
        getByExternalIdResultNodes.getLength() > 0
            && getByExternalIdResultNodes.item(0).getTextContent().trim().length() > 0;
