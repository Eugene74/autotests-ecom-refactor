...
import static com.ecom.core.config.EnvData.URL_ALIAS_2_INQUIRY;
...
public class InquiryAlias2 extends BaseTestAlias2 {
  ...
  public void testInquiryAlias2() throws Exception {
    ...
    HttpResponse response = sendPostRequest(URL_ALIAS_2_INQUIRY, xmlContent);
  ...
  private void verifyInquiryAliasResponse(String xml, SoftAssert softAssertion) throws Exception {
    ...
    boolean isInquiryAliasResultNotEmpty =
        inquiryAliasResultNodes.getLength() > 0
            && inquiryAliasResultNodes.item(0).getTextContent().trim().length() > 0;
    ...
    if (isCodePresent) {
      ...
      if (summaryNodes.getLength() > 0) {
        ...
        if (aliasesNotFoundNodes.getLength() > 0) {
          System.out.println(
              "aliasesNotFound: " + aliasesNotFoundNodes.item(0).getTextContent().trim());
        }
        if (aliasesRepeatedNodes.getLength() > 0) {
          System.out.println(
              "aliasesRepeated: " + aliasesRepeatedNodes.item(0).getTextContent().trim());
        }
