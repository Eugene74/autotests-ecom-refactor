...
import static com.ecom.core.config.EnvData.URL_ALIAS_2_DELETE_PAYMENT_CRED;
...
public class DeletePaymentCredentialAlias2 extends BaseTestAlias2 {

  private static final String XML_DELETE_PAYMENT_CREDENTIAL_ALIAS2_RESOURCE =
      "/XMLAlias2/DeletePaymentCredentialAlias2.xml";
  private static final int TIMEOUT_MS = 60000; // 60 секунд

  @Test(
      dependsOnMethods =
          "tests.com.Alias2.CreatePaymentCredentialAlias2.testCreatePaymentCredentialAlias2")
  public void testDeletePaymentCredentialAlias2() throws Exception {
    ...
    for (Map.Entry<String, String> entry : params.entrySet()) {
      xmlContent =
          xmlContent.replace(
              "${" + entry.getKey() + "}",
              Objects.requireNonNull(entry.getValue(), "Value for " + entry.getKey() + " is null"));
    }
    // Друк XML-запиту для налагодження
    System.out.println("Request XML: " + formatXml(xmlContent));
    ResponseData response = sendCustomDeleteRequest(URL_ALIAS_2_DELETE_PAYMENT_CRED, xmlContent);
  ...
  // todo move the method to a service class
  private ResponseData sendCustomDeleteRequest(String url, String xmlContent) throws Exception {
    ...
    HttpRequest request =
        HttpRequest.newBuilder()
            ...
            .method(
                "DELETE", HttpRequest.BodyPublishers.ofString(xmlContent, StandardCharsets.UTF_8))
            .build();
