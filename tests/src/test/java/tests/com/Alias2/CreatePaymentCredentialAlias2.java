...
import static com.ecom.core.config.EnvData.URL_ALIAS_2_CREATE_PAYMENT_CRED;
...
public class CreatePaymentCredentialAlias2 extends BaseTestAlias2 {
  ...
  public void testCreatePaymentCredentialAlias2() throws Exception {
    ...
    SharedDataStore.externalPaymentCredentialId =
        guid; // Зберігаємо GUID для подальшого використання
    ...
    HttpResponse response = sendPostRequest(URL_ALIAS_2_CREATE_PAYMENT_CRED, xmlContent);
  ...
  private void verifyCreatePaymentCredentialResponse(String xml, SoftAssert softAssertion)
      throws Exception {
    DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
    DocumentBuilder builder = factory.newDocumentBuilder();
    Document doc = builder.parse(new ByteArrayInputStream(xml.getBytes(StandardCharsets.UTF_8)));
    // Перевірка наявності елемента <CreatePaymentCredentialResult>
    NodeList createPaymentCredentialResultNodes =
        doc.getElementsByTagName("CreatePaymentCredentialResult");
    boolean isCreatePaymentCredentialResultNotEmpty =
        createPaymentCredentialResultNodes.getLength() > 0
            && createPaymentCredentialResultNodes.item(0).getTextContent().trim().length() > 0;
    System.out.println(
        "CreatePaymentCredentialResult is not empty: " + isCreatePaymentCredentialResultNotEmpty);
    softAssertion.assertTrue(
        isCreatePaymentCredentialResultNotEmpty, "<CreatePaymentCredentialResult> is empty");
    ...
    if (isCodePresent) {
      ...
      if (externalIdNodes.getLength() > 0) {
        ...
        SharedDataStore.paymentCredentialExternalId =
            externalIdValue; // Зберігаємо значення для подальшого використання
      }
      if (idNodes.getLength() > 0) {
        String idValue = idNodes.item(0).getTextContent().trim();
        System.out.println("Id: " + idValue);
        SharedDataStore.paymentCredentialId =
            idValue; // Зберігаємо значення для подальшого використання
      }
