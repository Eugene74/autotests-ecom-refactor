...
import static com.ecom.core.config.EnvData.ID_AVAL_5;
...
public class CrossBorder extends BaseUiTest {
  ...
  public void validateCrossBorder() throws Exception {
    ...
    try {
      ...
      String[] senderCardData = cardMCmta;
      ...
      WebElement senderCardExpiryDateInput =
          driver.findElement(By.id("sender-card-expiry-date-input"));
      ...
      WebElement sendercardholdernameinput =
          driver.findElement(By.id("sender-card-holder-name-input"));
      sendercardholdernameinput.sendKeys(senderName);

      WebElement recipientcardholdernameinput =
          driver.findElement(By.id("recipient-card-holder-name-input"));
      recipientcardholdernameinput.sendKeys(recipientName);

      // Дані картки отримувача
      WebElement recipientCardNumberInput =
          driver.findElement(By.id("recipient-card-number-input"));
      recipientCardNumberInput.sendKeys(cardVISArec.split(";")[0]);
      ...
      WebElement amountInput =
          wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("amount-input")));
      ...
      WebElement finalContinueButton =
          driver.findElement(
              By.xpath(
                  "//button[@type='button' and contains(@class, 'app-button_primary')]/span[contains(text(), 'Continue')]"));
      ...
      wait.until(
          ExpectedConditions.visibilityOfElementLocated(
              By.xpath(
                  "//p[@class='summary-form__fake-input-value' and text()='" + senderName + "']")));
      wait.until(
          ExpectedConditions.visibilityOfElementLocated(
              By.xpath(
                  "//p[@class='summary-form__fake-input-value' and text()='"
                      + recipientName
                      + "']")));
      ...
      WebElement transferButton =
          driver.findElement(
              By.xpath(
                  "//button[@type='button' and contains(@class, 'app-button_primary')]/span[contains(text(), 'Transfer')]"));
      ...
      WebElement nextTransferButton =
          wait.until(
              ExpectedConditions.visibilityOfElementLocated(
                  By.xpath(
                      "//a[@id='button-continue' and contains(@class, 'app-button_primary')]")));
      WebElement successfulTransferMessage =
          wait.until(
              ExpectedConditions.visibilityOfElementLocated(
                  By.xpath(
                      "//p[@class='status-page__description' and text()='Successful transfer']")));
      ...
      Assert.fail(
          "Сторінку не вдалося завантажити або знайти необхідні елементи: " + e.getMessage());
    }
  }
    ...
    try (Connection connection = JDBCConnection.getDBConnection();
      ...
      try (ResultSet resultSet = statement.executeQuery()) {
        if (resultSet.next()) {
          ...
          // Перевірка значень
          Assert.assertEquals(
              merchantCode, "1000148", "MERCHANT_CODE не відповідає очікуваному значенню");
      ...
      if (trackingId != null) {
        try (PreparedStatement tranStatement =
            connection.prepareStatement(
          ...
          try (ResultSet tranResultSet = tranStatement.executeQuery()) {
            if (tranResultSet.next()) {
              ...
              Assert.assertEquals(
                  purchaseDesc,
                  "P2P Cross-Border Transfer",
                  "PURCHASE_DESC не відповідає очікуваному значенню");
            }
          }
        }
      } else {
