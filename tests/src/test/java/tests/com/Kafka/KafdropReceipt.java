    ...
    public void receiptUPC() {
        ...
        try {
            ...
            WebElement dataRow = wait.until(
                    ExpectedConditions.visibilityOfElementLocated(By.xpath("//tr[@class='dataRow']/td/a[contains(@href, 'receiptUPC-" + pipelineId + "')]"))
            );
            ...
            if (topicName.equals("receiptUPC-" + pipelineId)) {
                ...
                try {
                    ...
                    WebElement viewMessagesButton = wait.until(
                            ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[@id='topic-messages' and contains(@href, 'receiptUPC-" + pipelineId + "')]"))
                    );
                    viewMessagesButton.click();
                    System.out.println("Знайдено і натиснуто на кнопку View Messages для topic: receiptUPC-" + pipelineId);
                } catch (Exception e) {
                    System.out.println("receiptUPC-" + pipelineId + " не знайдено або не співпадає в View Messages");
                ...
                while (hasNext) {
                    ...
                    List<WebElement> nextPageLinks = driver.findElements(By.cssSelector("li.page-item.disabled a.page-link.btn-success i.fa.fa-fast-forward"));
                    if (!nextPageLinks.isEmpty()) {
                        hasNext = false;
                        System.out.println("Далі переходити не можна, тому що наступна кнопка 'disabled'");
                    } else {
                        nextPageLinks = driver.findElements(By.cssSelector("a.page-link.btn-success i.fa.fa-fast-forward"));
                        if (!nextPageLinks.isEmpty()) {
                            WebElement nextPage = nextPageLinks.get(0); // Перша (і єдина) активна кнопка для переходу на наступну сторінку
                            nextPage.click();
