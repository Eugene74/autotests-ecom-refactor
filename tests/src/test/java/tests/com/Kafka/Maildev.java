...
import static com.ecom.core.config.EnvData.URL_MAIL_DEV;
...
public class Maildev extends EndpointBaseTest {
  ...
  public void emailSendTo() {
    ...
    try {
      WebElement pipelineElement =
          driver.findElement(
              By.xpath(
                  "//div[@class='dashboard__main-menu-item']//span[text()='"
                      + pipelineName
                      + "']/ancestor::div[contains(@class, 'dashboard__main-menu-item')]"));
      WebElement downChevronButton =
          pipelineElement.findElement(By.xpath(".//i[contains(@class, 'mdi-chevron-down')]"));
      ...
      for (WebElement emailItem : emailItems) {
        ...
        WebElement createdAtElement =
            emailItem.findElement(By.xpath(".//div[contains(text(), 'Created at: ')]"));
        ...
        if (toText.contains("viacheslav.yarosh@upc.ua")) {
          ...
          if (createdAtDate.equals(today)) {
            ...
            //  System.out.println("To: viacheslav.yarosh@upc.ua з не поточною датою: " +
            // createdAtText);
          }
        }
      }
      ...
      System.out.println(
          "Загальна кількість адресатів 'To: viacheslav.yarosh@upc.ua' з поточною датою: " + count);
