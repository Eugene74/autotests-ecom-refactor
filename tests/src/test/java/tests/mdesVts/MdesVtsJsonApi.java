...
public class MdesVtsJsonApi extends BaseApiTest {
  ...
  public void setParam() {
    setMerchantAtt(ID_AVAL_5, ALLOW_SUPPORT_TOKEN, "true");
    setMerchantAtt(ID_AVAL_5, ALLOW_TOKEN_MDES, "true");
    setMerchantAtt(ID_AVAL_5, ALLOW_TOKEN_VTS, "true");
  }

  @DataProvider(name = "cardProvider")
  public Object[][] cardProvider() {
    return new Object[][] {{"MASTERCARD"}, {"VISA"}};
  }
  ...
  private String signature =
      "EXDEhK9kMK0lwTEWH4mm1oJvKm5vVFyXnyDnqEDHDc3mYyXEhLv3Ih6_fdmN-apUPxgV5G"
  ...
  public void testCreateToken(String cardName) throws Exception {
    ...
    String jsonData =
        requests.prepareCreateTokenRequest(
            JSON_TEMPLATES_CREATE_TOKEN_MDES_VTS,
            trackingId,
            card,
            MERCHANT_ID_AVAL_5,
            TERMINAL_ID_AVAL_5);
    HttpResponse response = sendPostRequest(URL_MDES_VTS_CREATE_TOKEN, jsonData, header, signature);
  ...
  public void testRefreshToken(String cardName) throws Exception {
    ...
    MdesVtsRequests requests = new MdesVtsRequests();
    String jsonData =
        requests.prepareTokenActionRequest(
            JSON_TEMPLATES_REFRESH_TOKEN_MDES_VTS,
            trackingId,
            tokenID,
            MERCHANT_ID_AVAL_5,
            TERMINAL_ID_AVAL_5);
    ...
    HttpResponse response =
        sendPostRequest(URL_MDES_VTS_REFRESH_TOKEN, jsonData, header, signature);
    JSONObject payLoadJson = requests.getPayloadFromJSONResponse(response);
    String new_tokenID = payLoadJson.getString("UPCToken");
    assertEquals(
        new_tokenID,
        tokenID,
        "Actual tokenID" + new_tokenID + " is not equal to expected " + tokenID);
  ...
  public void testCardMetaData(String cardName) throws Exception {
    ...
    MdesVtsRequests requests = new MdesVtsRequests();
    String jsonData =
        requests.prepareCardMetadataRequest(
    ...
    HttpResponse response =
        sendPostRequest(URL_MDES_VTS_CARD_METADATA, jsonData, header, signature);
    JSONObject payLoadJson = requests.getPayloadFromJSONResponse(response);
    ...
    if (card == cardMdes) {
      assertEquals(
          contentType,
          "BrandLogoAsset",
          "Content type is " + contentType + " but expected BrandLogoAsset");
    } else if (card == cardVts) {
      assertEquals(
          contentType, "cardSymbol", "Content type is " + contentType + " but expected cardSymbol");
    }
  ...
  public void testCardContent(String cardName) throws Exception {
    ...
    MdesVtsRequests requests = new MdesVtsRequests();
    String jsonData =
        requests.prepareCardContentRequest(
    ...
    HttpResponse response = sendPostRequest(URL_MDES_VTS_CARD_CONTENT, jsonData, header, signature);
    JSONObject payLoadJson = requests.getPayloadFromJSONResponse(response);
  ...
  public void testTokenStatus(String cardName) throws Exception {
    ...
    HttpResponse response = sendPostRequest(URL_MDES_VTS_TOKEN_STATUS, jsonData, header, signature);
    JSONObject payLoadJson = requests.getPayloadFromJSONResponse(response);
    ...
    assertEquals(
        newTokenID,
        tokenID,
        "Actual tokenID" + newTokenID + " is not equal to expected " + tokenID);
  ...
  public void testTokenSuspend(String cardName) throws Exception {
    ...
    HttpResponse response =
        sendPostRequest(URL_MDES_VTS_TOKEN_SUSPEND, jsonData, header, signature);
    JSONObject payLoadJson = requests.getPayloadFromJSONResponse(response);
    ...
    String tokenStatus = payLoadJson.getString("status");
    assertEquals(
        tokenStatus, "SUSPENDED", "Token status is " + tokenStatus + " but expected SUSPENDED");
    System.out.println("Token Status: " + tokenStatus);
  ...
  public void testTokenResume(String cardName) throws Exception {
    ...
    HttpResponse response = sendPostRequest(URL_MDES_VTS_TOKEN_RESUME, jsonData, header, signature);
    JSONObject payLoadJson = requests.getPayloadFromJSONResponse(response);
  ...
  public void testTokenDelete(String cardName) throws Exception {
    ...
    HttpResponse response = sendPostRequest(URL_MDES_VTS_TOKEN_DELETE, jsonData, header, signature);
    JSONObject payLoadJson = requests.getPayloadFromJSONResponse(response);
    ...
    String tokenStatus = payLoadJson.getString("status");
    assertEquals(
        tokenStatus, "DEACTIVATED", "Token status is " + tokenStatus + " but expected DEACTIVATED");
    System.out.println("Token Status: " + tokenStatus);
