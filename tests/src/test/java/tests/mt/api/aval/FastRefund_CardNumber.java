...
import static com.ecom.core.config.CardConfig.cardMCfr;
...
import static com.ecom.tests.support.MoneyTransferRequests.fastRefundAPICard;
...
public class FastRefund_CardNumber {
  ...
  public void makePayment() {
    requestDoc = payment(cardMCmt, "payment FastRefund", MERCHANT_ID_AVAL, TERMINAL_ID_AVAL);
  ...
  public void makeFastRefundCard() {
    Document requestRFDoc =
        fastRefundAPICard(MERCHANT_ID_AVAL, TERMINAL_ID_AVAL, requestDoc, responseDoc);

    if (requestRFDoc.getElementsByTagName("ApprovalCode").getLength() > 0) {
      requestRFDoc
          .getElementsByTagName("ApprovalCode")
          .item(0)
          .setTextContent(approvalCodeFromPayment);
    }
    ...
    if (requestRFDoc.getElementsByTagName("Value").getLength() > 0) {
      String value =
          totalAmountFromPayment != null && !totalAmountFromPayment.isEmpty()
              ? totalAmountFromPayment
              : "100";
    ...
    if (requestRFDoc.getElementsByTagName("RecipientCardNumber").getLength() > 0) {
      requestRFDoc
          .getElementsByTagName("RecipientCardNumber")
          .item(0)
          .setTextContent(cardMCfr); // Відкритий номер карти
    }
    ...
    if (requestRFDocForLogging.getElementsByTagName("RecipientCardNumber").getLength() > 0) {
      ...
      requestRFDocForLogging
          .getElementsByTagName("RecipientCardNumber")
          .item(0)
          .setTextContent(maskedCardNumber);
    }
    ...
    Document responseRFDoc = sendRequest(URL_FR, requestRFDoc);
    ...
    softAssertion.assertEquals(
        getElementFromDocument(responseRFDoc, "Message"), "Approved", "Message");
    softAssertion.assertEquals(
        getElementFromDocument(responseRFDoc, "ApprovalCode").length(), 6, "ApprovalCode");
  ...
  public void makePaymentPartialFRefundCard() {
    Document requestDoc = payment(cardMC, "payment FastRefund", MERCHANT_ID_AVAL, TERMINAL_ID_AVAL);
  ...
  public void makePartialFastRefundCard() {
    ...
    if (requestRFDoc.getElementsByTagName("RecipientCardNumber").getLength() > 0) {
      requestRFDoc
          .getElementsByTagName("RecipientCardNumber")
          .item(0)
          .setTextContent(cardMCardfr); // Відкритий номер карти
    }
