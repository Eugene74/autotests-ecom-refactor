...
public class FastRefund_Token {
  ...
  public void makeFastRefundToken() {
    Document requestRFDoc =
        fastRefundAPIToken(MERCHANT_ID_AVAL, TERMINAL_ID_AVAL, requestDoc, responseDoc);
    ...
    if (requestRFDoc.getElementsByTagName("TokenId").getLength() > 0) {
      requestRFDoc
          .getElementsByTagName("TokenId")
          .item(0)
          .setTextContent(tokenIdFromPayment); // Вставляем токен как текст
    }
  ...
  public void makePartialFastRefundToken() {
    ...
    System.out.println(
        "--FAST REFUND REQUEST WITH HALF VALUE--\n" + convertXMLDocumentToString(requestRFDoc));
