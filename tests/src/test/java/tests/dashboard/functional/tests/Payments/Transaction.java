...
public class Transaction extends BaseUiTest {
  ...
  public void makePayment() {
    Document requestDoc =
        payment(cardVISAmtRev, "payment dashboard", MERCHANT_ID_AVAL, TERMINAL_ID_AVAL);
  ...
  public void searchByMultFields() {
    DashboardRequest request = new DashboardRequest();
    boolean searchByMerch =
        request.findPaymentByMultField(
            orderId, "YVPauto" + "", rrn, cardVISAmtRev[0], approval_code);
    Assert.assertEquals(searchByMerch, true, "Search result flag");
  }
